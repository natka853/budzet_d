...
def filter_expenses(request):
    if request.user.is_authenticated:
        ...
        return render(request, "filtrowanie/../templates/filtrujWydatki.html", {'queryset': wy, 'categories': categories, 'fig': graph_div})
    else:
        return render(request, "unlogged.html", {})
...
def filter_incomes(request):
    if request.user.is_authenticated:
        ...
        return render(request, "filtrowanie/../templates/filtrujDochod.html", {'queryset': do, 'sources': sources, 'fig': graph_div})
    else:
        return render(request, "unlogged.html", {})
    ...
    def get(self, request, *args, **kwargs):
        if request.user.is_authenticated:
            ...
            context = {'incomes': incomes,
                       'expenses': expenses,
                       'date': date.today()}
            response = render_to_pdf(template_name, context, filename)
            return response
        else:
            return render(request, "unlogged.html", {})
