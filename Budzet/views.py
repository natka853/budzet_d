...
def my_expenses(request, *args, **kwargs):
    if request.user.is_authenticated:
        expenses = Wydatek.objects.filter(kategoria__in=Kategoria.objects.filter(user=request.user.id)).order_by('data').reverse()
...
def register_admin(request):
    if request.method == 'POST':
        ...
        if form.is_valid(): # do wyrzucenia dwie linijki że is_staff i is_superuser = true !!!!!!!
            form.save()
